...
class _ProgressCardWidgetState extends State<ProgressCardWidget>
  ...
  Widget _buildTaskItem(Map<String, dynamic> task) {
    ...
    final taskName = task['name'] as String? ?? task['englishName'] ?? 'Unknown Task';
    final arabicName = task['arabicName'] as String? ?? '';
    final progress = task['progress'] as int? ?? task['currentCount'] ?? 0;
    final target = task['target'] as int? ?? task['targetCount'] ?? 1;
    final taskType = task['type'] as String? ?? 'checkbox';
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Icon(
            _getTaskStatusIcon(isCompleted, progress, target, taskType),
            color: _getTaskStatusColor(isCompleted, progress, target),
            size: 16,
          ),
        ],
  ...
  IconData _getTaskStatusIcon(bool isCompleted, int progress, int target, String taskType) {
    if (isCompleted || progress >= target) {
      return Icons.check_circle;
    } else if (progress > 0) {
      return taskType == 'counter' ? Icons.pause_circle : Icons.radio_button_checked;
    } else {
      return taskType == 'counter' ? Icons.play_circle_outline : Icons.radio_button_unchecked;
    }
  }
